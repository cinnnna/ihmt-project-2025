% Z-spectrum simulation - Combined 1-band and 2-band comparison with normalization
%% pulse parameters
...
%% Calculate reference M0 for normalization
...
Mz_vec_1band_norm = Mz_vec_1band / M0f;
Mz_vec_2band_norm = Mz_vec_2band / M0f;

%% Plot both cases on the same diagram
figure('DockControls','on');
hold on;
% Plot 1-band case
plot(offset_vec/1e3, Mz_vec_1band_norm, 'b-', 'LineWidth', 1.6, 'DisplayName', '1-band');
% Plot 2-band case
plot(offset_vec/1e3, Mz_vec_2band_norm, 'r-', 'LineWidth', 1.6, 'DisplayName', '2-band');
% Formatting
xlabel('\Delta (kHz)');
ylabel('$|M_{z,\mathrm{free}}|$ after RF pulse (normalized)', 'Interpreter','latex');
...
figure('DockControls','on');
ihMT_diff = Mz_vec_1band_norm - Mz_vec_2band_norm; % ihMT contrast
...
fprintf('1-band Mz range (normalized): %.4f to %.4f\n', min(Mz_vec_1band_norm), max(Mz_vec_1band_norm));
fprintf('2-band Mz range (normalized): %.4f to %.4f\n', min(Mz_vec_2band_norm), max(Mz_vec_2band_norm));
